...
%% Legend
subplot(1,2,1);
legend([lines_prw_com_x, lines_prw_cp_x, lines_prw_cop_x, lines_com_control_x, lines_real_com_x, lines_real_cop_x, lines_support_x(1)], 'com_{prw}^x','cp_{prw}^x','cop_{prw}^x','jerk_{prw}^x','com_{real}^x','cop_{real}^x','new support');
xlabel('Time [s]');
ylabel('X [m]');
title('CoM Positions (previewed and realized)');
subplot(1,2,2);
legend([lines_prw_com_y, lines_prw_cp_y, lines_prw_cop_y, lines_com_control_y, lines_real_com_y, lines_real_cop_y, lines_support_y(1), lines_com_control_y], 'com_{prw}^y','cp_{prw}^y','cop_{prw}^y','jerk_{prw}^y','com_{real}^y','cop_{real}^y','new support');
xlabel('Time [s]');
ylabel('Y [m]');
title('CoM Positions (previewed and realized)');
